...
void
group_set_transform(group_t* group, matrix_t* matrix)
{
	matrix_t* old_matrix;
...
void
shape_set_texture(shape_t* shape, image_t* image)
{
	image_t* old_image;
	
	old_image = shape->texture;
	shape->texture = ref_image(image);
...
void
shape_draw(shape_t* shape, image_t* target, matrix_t* matrix, shader_t* shader)
{
	if (target != NULL)
		al_set_target_bitmap(get_image_bitmap(target));
	apply_shader(shader != NULL ? shader : get_default_shader());
